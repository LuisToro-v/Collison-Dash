...
var hero = {
  speed: 70, // Movement speed in pixels per second.
  x: 0,
  y: 0
};
...
function reset() {
  ...
  finish.x = Math.floor((Math.random() * BACKGROUND_IMG.length));
  finish.y = Math.floor((Math.random() *BACKGROUND_IMG.length));
}
...
function update(canvas, duration) {
  ...
  if (hero.x < finish.width & hero.y < finish.height){
    wins++;
